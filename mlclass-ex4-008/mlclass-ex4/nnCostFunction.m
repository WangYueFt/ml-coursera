...
for i = 1:m
	...
	yi = 1:num_labels;
	yi = y(i, :) == yi;
	J = J + (-yi*logh - (ones(size(yi), 1)-yi)*log(ones(size(h))-h));
	delta3 = h - yi';
	delta2 = Theta2'*delta3;
	delta2 = delta2(2:end);
	delta2 = delta2.*sigmoidGradient(Theta1*X(i, :)');
	Del2 = Del2 + delta3*(a2)';
	Del1 = Del1 + delta2*X(i, :);
end
...
J = J + lambda/2/m*(sum(sum(Theta1.*Theta1, 1), 2)+sum(sum(Theta2.*Theta2, 1), 2)...
	-sum(Theta1(:, 1).*Theta1(:, 1), 1) -sum(Theta2(:, 1).*Theta2(:, 1), 1));
